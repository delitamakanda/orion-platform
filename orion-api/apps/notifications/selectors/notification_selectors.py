...
class NotificationSelectors:
    ...
    @classmethod
    def mark_notification_as_read(cls, notification):
        return NotificationService.mark_as_read(notification)
    
    @staticmethod
    def get_unread_notifications_count_for_user(cls, user):
        return Notification.objects.filter(recipient=user, is_read=False).order_by('-created_at').count()
    
    @staticmethod
    def mark_all_notifications_as_read_for_user(cls, user):
